...
public class SoftVoxelRobot : AnimatBody
{
    ...
    public override void MotorEffect(Animat animat)
    {
        ...
        else if (animat.mind is NARS nar)
        {
            ...
            for (int i = 0; i < this.genome.voxel_array.Length; i++)
            {
                ...
                var st = (StatementTerm)Term.from_string("((*,{SELF}) --> outputNeuron" + cnt + ")");
     
                float NARSactivation = nar.GetGoalActivation(st);


                nnDecoderIn.Add(NARSactivation);

                decoderVoxels.Add(cvx_voxel);
                cnt++;
            }

            if (nnDecoderIn.Count != 8) Debug.LogError("error");

            int cycle = nar.current_cycle_number;
            float[] decOut = nar.genome.neuralGeneDecoder.Forward(nnDecoderIn.ToArray());
            if(decoderVoxels.Count != 8) Debug.LogError("error");
            for (int i = 0; i < decOut.Length; i++)
            {
                CVX_Voxel voxel = decoderVoxels[i];
                float activation = decOut[i];
                soft_voxel_object.SetVoxelTemperatureFromNeuronActivation(voxel, activation);
            }
    ...
    List<float> nnDecoderIn = new();
    List<CVX_Voxel> decoderVoxels = new();
    ...
    public override void Sense(Animat animat)
    {
        ...
        else if (animat.mind is NARS nar)
        {
            ...
            for (int i = 0; i < this.genome.voxel_array.Length; i++)
            {
                ...
                bool is_touching_ground = VoxelyzeEngine.GetVoxelFloorPenetration(cvx_voxel) > 0;
                nnEncoderIn.Add(is_touching_ground ? 1.0f : -1.0f);
            }
            ...
            for (int i = 0; i < this.genome.voxel_array.Length; i++)
            {
                ...
                temp[1] = 0;
                temp[2] = 0;
                VoxelyzeEngine.GetVoxelTemperature(cvx_voxel, temp);
                temp[0] /= (double)SoftVoxelObject.TEMP_MAX_MAGNITUDE;
                temp[1] /= (double)SoftVoxelObject.TEMP_MAX_MAGNITUDE;
                temp[2] /= (double)SoftVoxelObject.TEMP_MAX_MAGNITUDE;
                float activation = (float)(temp[0] + temp[1] + temp[2]) / 3f;
                nnEncoderIn.Add(activation);
            }
            if (nnEncoderIn.Count != 16) Debug.LogError("error");
            int cycle = nar.current_cycle_number;
            float[] encOut = nar.genome.neuralGeneEncoder.Forward(nnEncoderIn.ToArray());
            for (int i = 0; i < encOut.Length; i++)
            {
                var st = (StatementTerm)Term.from_string("(inputNeuron" + i + " --> Activated)");
                float f = 0.5f * (encOut[i] + 1f);
                float c = 0.99f;
                nar.SendInput(new Judgment(nar, st, new(f, c), cycle));
            }

        }
    }

    List<float> nnEncoderIn = new();
